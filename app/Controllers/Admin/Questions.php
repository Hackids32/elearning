    ...
    public function edit($id_berita)
    {
        ...
        if ($this->request->getMethod() === 'post' && $this->validate(
            [
                'judul_berita'     => 'required',
                'gambar'         => [
                    'mime_in[gambar,image/jpg,image/jpeg,image/gif,image/png,video/mp4]',
                    'max_size[gambar,4096]',
                ],
            ]
        )) {
            if (!empty($_FILES['gambar']['name'])) {
                ...
                $m_berita->edit($data);
                return redirect()->to(base_url('admin/questions/edit/' . $this->request->getVar('id_berita')))->with('Success', 'Data Berhasil di Simpan');
            } else {
                $data = array(
                    'id_berita'        => $id_berita,
                    'id_user'        => $this->session->get('id_user'),
        ...
        $data = [
            'title'            => 'Edit Materi: ' . $berita['judul_berita'],
            'kategori'        => $kategori,
            'berita'        => $berita,
            'content'        => 'admin/questions/edit'
        ];
        echo view('admin/layout/wrapper', $data);
    }
